...
/**
 * This class is responsible for communicating with the Paydoor API
 */
class Paydoor
{
    const BASE_URL = 'https://www.paydoor.co';
    const NEW_ADDRESS_URL = 'https://www.paydoor.co/api/new_address';
    const PRICE_URL = 'https://www.paydoor.co/api/price';
    const SET_CALLBACK_URL = 'https://www.paydoor.co/api/update_callback';
    const GET_CALLBACKS_URL = 'https://www.paydoor.co/api/address?&no_balance=true&only_xpub=true&get_callback=true';
    const TEMP_API_KEY_URL = 'https://www.paydoor.co/api/temp_wallet';
    const TEMP_WITHDRAW_URL = 'https://www.paydoor.co/api/temp_withdraw_request';

    const BCH_BASE_URL = 'https://bch.paydoor.co';
    const BCH_NEW_ADDRESS_URL = 'https://bch.paydoor.co/api/new_address';
    const BCH_PRICE_URL = 'https://bch.paydoor.co/api/price';
    const BCH_SET_CALLBACK_URL = 'https://bch.paydoor.co/api/update_callback';
    const BCH_GET_CALLBACKS_URL = 'https://bch.paydoor.co/api/address?&no_balance=true&only_xpub=true&get_callback=true';
    ...
    public function get_api_key()
    {
        ...
        if (!$api_key)
        {
            $api_key = get_option("paydoor_temp_api_key");
        }
        return $api_key;
    }
    ...
    public function new_address($secret, $crypto, $reset=false)
    {
        ...
        if($crypto === 'btc'){
            $url = Paydoor::NEW_ADDRESS_URL.$get_params;
        }else{
            $url = Paydoor::BCH_NEW_ADDRESS_URL.$get_params;            
        }
    ...
    {
        if($crypto === 'btc'){
            $url = Paydoor::PRICE_URL. "?currency=$currency";
        }else{
            $url = Paydoor::BCH_PRICE_URL. "?currency=$currency";
        }
    ...
    {
        if ($crypto === 'btc'){
            $url = Paydoor::SET_CALLBACK_URL;
        }else{
            $url = Paydoor::BCH_SET_CALLBACK_URL;
        }
    ...
    {
        if ($crypto === 'btc'){
            $url = Paydoor::GET_CALLBACKS_URL;
        }else{
            $url = Paydoor::BCH_GET_CALLBACKS_URL;
        }
    ...
    public function check_get_callbacks_response_code($response){
        ...
        if (!wp_remote_retrieve_response_code($response)) {
            $error_str = __('Your server is blocking outgoing HTTPS calls', 'paydoor-bitcoin-payments');
        }
        elseif (wp_remote_retrieve_response_code($response)==401)
            $error_str = __('API Key is incorrect', 'paydoor-bitcoin-payments');
    ...
    public function check_get_callbacks_response_body ($response, $crypto){
        ...
        //if merchant doesn't have any xPubs on his Paydoor account
        if (!isset($response_body) || count($response_body) == 0)
        {
            $error_str = __('Please add a new store on paydoor website', 'paydoor-bitcoin-payments');
        }
        //if merchant has at least one xPub on his Paydoor account
    ...
    {
        $callback_secret = get_option('paydoor_callback_secret');
        $api_url = WC()->api_request_url('WC_Gateway_Paydoor');
    ...
    public function check_callback_urls_or_set_one($crypto, $response) 
    {
        ...
        if (!$api_key && $crypto === 'bch'){
            $error_str = __('Set the API Key or disable BCH', 'paydoor-bitcoin-payments');
            return $error_str;
        }
    ...
    /*
     * Get list of crypto currencies supported by Paydoor
     */
    ...
    public function getActiveCurrencies() {
        ...
        $paydoor_currencies = $this->getSupportedCurrencies();
        foreach ($paydoor_currencies as $code => $currency) {
            $enabled = get_option('paydoor_'.$code);
    ...
    public function make_withdraw()
    {
        ...
        if (get_option('paydoor_temp_withdraw_amount') > 0)
        {

            $url = Paydoor::TEMP_WITHDRAW_URL.'?tempkey='.$temp_api_key;
            $response = $this->post($url, $api_key);
            $responseObj = json_decode(wp_remote_retrieve_body($response));
            $response_code = wp_remote_retrieve_response_code($response);
            if ($response_code != 200)
            {
                $message = __('Error while making withdraw: '.$responseObj->message, 'paydoor-bitcoin-payments');
                return [$message, 'error'];
            }
            update_option("paydoor_temp_api_key", null);
            update_option('paydoor_temp_withdraw_amount', 0);
            $message = __('Your funds withdraw request has been submitted. Please check your Paydoor registered emailid for details', 'paydoor-bitcoin-payments');
    ...
    private function get($url, $api_key = '')
    {
        ...
        if(is_wp_error( $response )){
           ...
           echo __('Something went wrong', 'paydoor-bitcoin-payments').': '.$error_message;
        }else{
            return $response;
        }
    }
    ...
    // Runs when the Paydoor Test Setup button is clicked
    ...
    public function get_parameterized_wc_url($params = array()){
        $order_url = WC()->api_request_url('WC_Gateway_Paydoor');
    ...
    // Adds the header to the paydoor page
    public function load_paydoor_header($template_name, $additional_script=NULL){
    ...
    // Adds the footer to the paydoor page
    public function load_paydoor_footer($template_name){
    ...
    // Adds the selected template to the paydoor page
    public function load_paydoor_template($template_name, $context = array(), $additional_script = NULL){
        $this->load_paydoor_header($template_name, $additional_script);

        // Load the selected template
        $template = 'paydoor_'.$template_name.'.php';
        ...
        $this->load_paydoor_footer($template_name);
    ...
    public function calculate_order_params($order){
        ...
        if ( $order['status'] == -1) {
            ...
            if(get_woocommerce_currency() != 'BTC'){
                ...
                $price = $price * 100/(100+get_option('paydoor_margin', 0));
            }else{
                $price = 1;
            }
    ...
    public function create_new_order($order_id, $crypto){
        $responseObj = $this->new_address(get_option("paydoor_callback_secret"), $crypto);
        if($responseObj->response_code != 200) {
    ...
    public function get_error_context($error_type){
        ...
        if ($error_type == 'generic') {
            ...
            $context['error_title'] = __('Could not generate new address (This may be a temporary error. Please try again)', 'paydoor-bitcoin-payments');
            $context['error_msg'] = __('If this continues, please ask website administrator to do following:<br/><ul><li>Login to admin panel, navigate to Settings > Paydoor > Currencies and click Test Setup to diagnose the exact issue.</li><li>Check paydoor registered email address for error messages</li>', 'paydoor-bitcoin-payments');
        } else if($error_type == 'underpaid') {
            $context['error_title'] = '';
            $context['error_msg'] = __('Paid order BTC amount is less than expected. Contact merchant', 'paydoor-bitcoin-payments');
        }
    ...
    public function get_checkout_script($context, $template_name) {
        ...
        if ($template_name === 'checkout') {
            $script = "const paydoor_data = '" . json_encode( array (
                'crypto' => $context['crypto'],
                'crypto_address' => $context['order']['address'],
                'time_period' => get_option('paydoor_timeperiod', 10),
    ...
    public function load_checkout_template($order_id, $crypto){
        ...
        // Load the template
        $this->load_paydoor_template($template_name, $context, $script);
    }
    ...
    public function get_order_by_id_and_crypto($order_id, $crypto){
        global $wpdb;
        $order = $wpdb->get_row(
            $wpdb->prepare("SELECT * FROM ".$wpdb->prefix."paydoor_orders WHERE order_id = %s AND crypto = %s", array($order_id, $crypto)),
            ARRAY_A
        );
        if($order){
            return $order;
    ...
    // Inserts a new order in paydoor_orders table
    ...
    // Updates an order in paydoor_orders table
    ...
    public function process_order($order_id, $crypto){
        ...
        if ($order) {
            ...
            if (!$this->insert_order($order)) {
                ...
                return array("error"=>__("Duplicate Address Error. This is a Temporary error, please try again", 'paydoor-bitcoin-payments'));
            }
    ...
    public function get_order_by_address($address){
        global $wpdb;
        $order = $wpdb->get_row(
            $wpdb->prepare("SELECT * FROM ".$wpdb->prefix."paydoor_orders WHERE address = %s", array($address)),
            ARRAY_A
        );
        if($order){
            return $order;
        }
        exit(__("Error: Paydoor order not found", 'paydoor-bitcoin-payments'));
    }
    ...
    public function check_callback_secret($secret){
        ...
        exit(__("Error: secret does not match", 'paydoor-bitcoin-payments'));
    }

    public function save_transaction($value, $order, $wc_order){
        if (!metadata_exists('post',$wc_order->get_id(),'paydoor_'. $order['crypto'] .'_txid', $order['txid']) )  {
            update_post_meta($wc_order->get_id(), 'paydoor_'. $order['crypto'] .'_txid', $order['txid']);
    ...
    public function update_paid_amount($status, $value, $order, $wc_order){
        $network_confirmations = get_option("paydoor_network_confirmation",2);
    ...
    public function check_paid_amount($status, $value, $order, $wc_order){
      $underpayment_slack = get_option("paydoor_underpayment_slack", 0)/100 * $order['satoshi'];
      if ($order['satoshi'] - $underpayment_slack > $value) {
        $status = -2; //Payment error , amount less than expected 
        $wc_order->update_status('failed', __('Paid amount less than expected.', 'paydoor-bitcoin-payments'));
      }else{
        $wc_order->add_order_note(__('Payment completed', 'paydoor-bitcoin-payments'));
        $wc_order->payment_complete($order['txid']);
      }
      if ($order['satoshi'] < $value) {
        $wc_order->add_order_note(__( 'Paid amount more than expected.', 'paydoor-bitcoin-payments' ));
      }
      return $status;
    }
    ...
    public function update_temp_draw_amount($value){
        if(get_option('paydoor_temp_api_key') && !get_option("paydoor_api_key")) {
            $current_temp_amount = get_option('paydoor_temp_withdraw_amount');
            $new_temp_amount = $current_temp_amount + $value;
            update_option('paydoor_temp_withdraw_amount', $new_temp_amount);
        }
    }

    // Process the paydoor callback
    public function process_callback($secret, $address, $status, $value, $txid, $rbf){
        ...
        if (empty($wc_order)) {
            exit(__("Error: Woocommerce order not found", 'paydoor-bitcoin-payments'));
        }
        ...
        if (!$rbf){
          ...
          // https://insights.paydoor.co/bitcoin-payments-can-now-easily-cancelled-a-step-forward-or-two-back/ 
    ...
    public function encrypt_hash($input)
    {
        ...
        $secret = get_option('paydoor_callback_secret');;
    ...
    public function decrypt_hash($hash)
    {
        ...
        if (strlen($hash) % 2 || !ctype_xdigit($hash)) {
            echo __("Error: Incorrect Hash. Hash cannot be validated.", 'paydoor-bitcoin-payments');
            exit();
        }
        ...
        if (empty(wc_get_order($decrypted))) {
            echo __("Error: Incorrect hash. Woocommerce order not found.", 'paydoor-bitcoin-payments');
            exit();
        }
