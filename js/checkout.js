...
class Paydoor {
    ...
    init() {
        ...
        if (!this.container) {
            throw Error(`Paydoor Initialisation Error: Container #${this.checkout_id} was not found!`)
        }

        // Load data attributes
        // This assumes a constant/var `paydoor_data` is defined before the script is called.
        try {
            this.data = JSON.parse(paydoor_data)
        } catch(e) {
            if (e.toString().includes('ReferenceError')) {
                throw Error(`Paydoor Initialisation Error: Data Object was not found in Window. Please set paydoor_data variable.`)
            }
            throw Error(`Paydoor Initialisation Error: Data Object is not a valid JSON.`)
        }
    ...
    connect_to_ws() {
        ...
        var ws = new ReconnectingWebSocket("wss://" + (this.data.crypto.code == 'btc' ? 'www' : this.data.crypto.code) + ".paydoor.co/payment/" + this.data.crypto_address);
        let $this = this
