...
angular.module('PaydoorApp', ['ngResource', 'monospaced.qrcode'])
...
function CheckoutController($scope, $interval, Order, $timeout, Url) {
    ...
    var paydoor_time_period = time_period_div.dataset.time_period;
    var totalTime = paydoor_time_period * 60;
    ...
    function proccess_order_data() {
        ...
        if ($scope.order.status == -1) {
            ...
            var ws = new ReconnectingWebSocket("wss://" + subdomain + ".paydoor.co/payment/" + $scope.order.address);
            ws.onmessage = function(evt) {
                ws.close();
                $interval(function() {
                    //Redirect to order confirmation page if message from socket
                    window.location = Url.get_wc_endpoint({'finish_order' : $scope.order_hash});
    ...
    //Check if the paydoor order is present
    function check_paydoor_order() {
        $scope.spinner = true;
        $scope.address_error = [];
    ...
    $scope.paydoor_address_click = function() {
        copy_to_clipboard("bnomics-address-copy");
    }

    //Copy bitcoin amount to clipboard
    $scope.paydoor_amount_click = function() {
